...
export default function PetTable({
  ...
  const [likelihoodKcValues, setLikelihoodKcValues] = useState<{
    [key: string]: string;
  }>({});
  const [likelihoodValues, setLikelihoodValues] = useState<{
    [key: string]: string;
  }>({});
  // Dual KC states for pets with two rates
  const [phosaniKcValues, setPhosaniKcValues] = useState<{
    [key: string]: string;
  }>({});
  const [branSacrificeKcValues, setBranSacrificeKcValues] = useState<{
    [key: string]: string;
  }>({});
  const [yamiContractsKcValues, setYamiContractsKcValues] = useState<{
    [key: string]: string;
  }>({});
  const [nidDestroyKcValues, setNidDestroyKcValues] = useState<{
    [key: string]: string;
  }>({});
  const [youngllefCorruptedKcValues, setYoungllefCorruptedKcValues] = useState<{
    [key: string]: string;
  }>({});
  const [vetionCalvarionKcValues, setVetionCalvarionKcValues] = useState<{
    [key: string]: string;
  }>({});
  const [callistoArtioKcValues, setCallistoArtioKcValues] = useState<{
    [key: string]: string;
  }>({});
  const [venenatisSpindelKcValues, setVenenatisSpindelKcValues] = useState<{
    [key: string]: string;
  }>({});
  const [lilZikHardKcValues, setLilZikHardKcValues] = useState<{
    [key: string]: string;
  }>({});
  const [jalNibRekOfftaskKcValues, setJalNibRekOfftaskKcValues] = useState<{
    [key: string]: string;
  }>({});
  const [tzrekJadOfftaskKcValues, setTzrekJadOfftaskKcValues] = useState<{
    [key: string]: string;
  }>({});
  // Quad KC for Dom
  ...
  const [dom8plusKcValues, setDom8plusKcValues] = useState<{
    [key: string]: string;
  }>({});
  // Quad KC for Tumeken's guardian
  const [tumeken150KcValues, setTumeken150KcValues] = useState<{
    [key: string]: string;
  }>({});
  const [tumeken300KcValues, setTumeken300KcValues] = useState<{
    [key: string]: string;
  }>({});
  const [tumeken400KcValues, setTumeken400KcValues] = useState<{
    [key: string]: string;
  }>({});
  const [tumeken500KcValues, setTumeken500KcValues] = useState<{
    [key: string]: string;
  }>({});
    ...
    async function fetchPetHours() {
      try {
        const response = await axios.get(
          proxyUrl + encodeURIComponent(hoursUrl),
        );
  ...
  const getPetIconClass = (petName: string, pets: PetData) => {
    ...
    return Object.entries(pets).find(
      ([name, count]) => name === petName && count === 1,
    )
      ? "obtained-pet-icon"
      : "pet-icon";
  };
  ...
  const handlePetClick = (petName: string) => {
    ...
    setManualPets((prevState) => {
      ...
      const totalPetHours = isExcluded
        ? prevState.pet_hours
        : prevState.pet_hours +
          (prevState.pets[petName] ? -newPetHours : newPetHours);
      return {
        ...prevState,
        pets: {
          ...prevState.pets,
  ...
  useEffect(() => {
    ...
    Object.keys(likelihoodKcValues).forEach((petName) => {
      function dualRateCalc(
        ...
        const secondaryRateObj = petRates.find(
          (r: any) => r.pet === secondaryName,
        );
        const primaryDropRate = primaryRateObj
          ? Number(primaryRateObj.dropRate)
          : null;
        const secondaryDropRate = secondaryRateObj
          ? Number(secondaryRateObj.dropRate)
          : null;
      ...
      if (petName === "Little nightmare") {
        newLikelihoodValues[petName] = dualRateCalc(
          "Little nightmare",
          "Phosani",
          likelihoodKcValues[petName],
          phosaniKcValues[petName],
          petRates,
        );
        ...
        newLikelihoodValues[petName] = dualRateCalc(
          ...
          "Nid (Destroy)",
          likelihoodKcValues[petName],
          nidDestroyKcValues[petName],
          petRates,
        );
      } else if (petName === "Youngllef") {
        newLikelihoodValues[petName] = dualRateCalc(
          "Youngllef (Normal Gauntlet)",
          "Youngllef (Corrupted Gauntlet)",
          likelihoodKcValues[petName],
          youngllefCorruptedKcValues[petName],
          petRates,
        );
      } else if (petName === "Vet'ion jr. ") {
        newLikelihoodValues[petName] = dualRateCalc(
          "Vet'ion jr. ",
          "Vet'ion jr. (Calvar'ion)",
          likelihoodKcValues[petName],
          vetionCalvarionKcValues[petName],
          petRates,
        );
        ...
        newLikelihoodValues[petName] = dualRateCalc(
          ...
          "Callisto cub (Artio)",
          likelihoodKcValues[petName],
          callistoArtioKcValues[petName],
          petRates,
        );
      } else if (petName === "Venenatis spiderling") {
        newLikelihoodValues[petName] = dualRateCalc(
          "Venenatis spiderling",
          "Venenatis spiderling (Spindel)",
          likelihoodKcValues[petName],
          venenatisSpindelKcValues[petName],
          petRates,
        );
        ...
        newLikelihoodValues[petName] = dualRateCalc(
          ...
          "Jal-nib-rek (Offtask)",
          likelihoodKcValues[petName],
          jalNibRekOfftaskKcValues[petName],
          petRates,
        );
        ...
        newLikelihoodValues[petName] = dualRateCalc(
          ...
          "Tzrek-jad (Offtask)",
          likelihoodKcValues[petName],
          tzrekJadOfftaskKcValues[petName],
          petRates,
        );
      } else if (petName === "Tumeken's guardian") {
        newLikelihoodValues[petName] = quadRateCalc(
          [
            "Tumeken's guardian 150",
            "Tumeken's guardian 300",
            "Tumeken's guardian 400",
            "Tumeken's guardian 500",
          ],
          [
        ...
        if (dropRate && kc > 0) {
          ...
          newLikelihoodValues[petName] =
            x < 10 ? x.toFixed(2) + "x" : Math.round(x) + "x";
        } else {
          newLikelihoodValues[petName] = "";
        }
      }
    ...
    tumeken400KcValues,
    tumeken500KcValues,
    tzrekJadOfftaskKcValues,
    jalNibRekOfftaskKcValues,
    phosaniKcValues,
    branSacrificeKcValues,
    yamiContractsKcValues,
    nidDestroyKcValues,
    youngllefCorruptedKcValues,
    vetionCalvarionKcValues,
    callistoArtioKcValues,
    venenatisSpindelKcValues,
    lilZikHardKcValues,
    dom6KcValues,
    dom7KcValues,
    dom8KcValues,
  ...
  const handleExportPetData = () => {
    ...
    const petsExport = Object.keys(exportSource).reduce(
      (acc, key) => {
        const petCount = exportSource[key];
        const obtainedPets = Object.keys(petCount.pets).filter(
          (petName) => petCount.pets[petName] === 1,
        );
        const petData = obtainedPets.reduce(
          (petAcc, petName) => {
    ...
    if (storedData) {
      try {
        ...
        if (importedData.likelihoodKcValues) {
          ...
          Object.values(importedData.likelihoodKcValues).forEach(
            (kcObj: any) => {
        ...
        if (manualMode) {
          ...
          const newPetCount = Object.values(updatedPets).filter(
            (v) => v === 1,
          ).length;
          setManualPets((prev) => ({
            ...prev,
        ...
        if (importedData.petCountColor)
          onSetPetCountColor(importedData.petCountColor);
        if (importedData.petHoursColor)
          onSetPetHoursColor(importedData.petHoursColor);
        if (importedData.petBgColor1)
          onSetPetBgColor1(importedData.petBgColor1);
        if (importedData.petBgColor2)
          onSetPetBgColor2(importedData.petBgColor2);
        ...
        if (typeof importedData.hideAvatar === "boolean")
          onSetHideAvatar(importedData.hideAvatar);
        if (typeof importedData.isCompact === "boolean")
          onSetIsCompact(importedData.isCompact);
      } catch (e) {
        // Ignore parse errors
      }
    }
  ...
  const renderPetBox = (petName: string, pets: PetData) => {
    ...
    const petImage = detailedMode
      ? DetailedPet[formatPetName(petName) as keyof typeof DetailedPet]
      : InvyPet[formatPetName(petName) as keyof typeof InvyPet];
    return (
      <Box
      ...
      >
        <Box
          className={petIconClass}
          onClick={() => manualMode && handlePetClick(petName)}
        >
          <img
            src={petImage}
            alt={petName}
            className={detailedMode ? "detailed-pet-image" : undefined}
          />
        </Box>
        ...
        {likelihoodMode &&
          !isGroup &&
          petIconClass === "obtained-pet-icon" &&
          (() => {
            ...
            return (
              <Typography
                variant="body2"
                className="likelihood-mode-text"
                sx={{ color: rateColor }}
              >
  ...
  const renderPetGrid = (
    ...
    const visiblePets = petNames
      .map((petName) => renderPetBox(petName, pets))
      .filter((petBox) => petBox !== null);
    ...
    return (
      <Grid sx={{ pl: 2, pr: 3, pb: 1.5, pt: 1.5 }}>
        ...
        <Box className="pet-grid">{visiblePets}</Box>
      </Grid>
    );
  };

  const renderMissingPets = (pets: PetData) => {
    const missingPets = Object.keys(pets).filter(
      (petName) => getPetIconClass(petName, pets) === "pet-icon",
    );
  ...
  const renderObtainedPets = (
    category: string,
    petNames: string[],
    pets: PetData,
  ) => {
    const obtainedPets = petNames.filter(
      (petName) => getPetIconClass(petName, pets) === "obtained-pet-icon",
    );
  ...
  const petRates = [
    ...
    { main: "55", iron: "60", dropRate: "2000", pet: "Venenatis spiderling" },
    {
      ...
      iron: "18",
      dropRate: "2800",
      pet: "Venenatis spiderling (Spindel)",
    },
    ...
    { main: "100", iron: "50", dropRate: "3000", pet: "Sraracha" },
    {
      ...
      iron: "50",
      dropRate: "800",
      pet: "Youngllef (Corrupted Gauntlet)",
    },
    {
      main: "10",
      iron: "56",
      dropRate: "2000",
      pet: "Youngllef (Normal Gauntlet)",
    },
    ...
    { main: "0", iron: "0", dropRate: "250", pet: "Dom 8 plus" },
    {
      ...
      iron: "3.1",
      dropRate: "1618",
      pet: "Tumeken's guardian 150",
    },
    {
      main: "2.6",
      iron: "2.6",
      dropRate: "733",
      pet: "Tumeken's guardian 300",
    },
    { main: "2", iron: "2", dropRate: "315", pet: "Tumeken's guardian 400" },
    {
      ...
      iron: "1.3",
      dropRate: "192",
      pet: "Tumeken's guardian 500",
    },
    { main: "0", iron: "0", dropRate: "3000", pet: "Gull" },
    { main: "0", iron: "0", dropRate: "5000", pet: "Soup" },
    { main: "0", iron: "0", dropRate: "1000", pet: "Beef" },
  ...
  const getTotalPetHours = () => {
    ...
    if (
      ...
      petCounts["1"] &&
      petCounts["1"].pets &&
      Object.keys(petHoursMap).length > 0
    ) {
      return Object.entries(petCounts["1"].pets)
        .filter(
          ([petName, obtained]) =>
            obtained === 1 && petHoursMap[petName] !== undefined,
        )
            ...
            <div key={key}>
                        ...
                        <Box sx={{ display: "flex", flexGrow: "0" }}>
                          <div style={{ height: "160px", width: "120px" }}>
                            <CircularProgressbar
                              value={
                                manualMode
                                  ? manualPets.pet_count / totalPets
                                  : petCount.pet_count / totalPets
                              }
                              maxValue={1}
                          ...
                          >
                            ...
                            >
                              <div style={{ height: "170px", width: "200px" }}>
                                <img
                                  src={
                                    getTrophyImage(petCount.rank) || otherTrophy
                                  }
                                  alt="trophy"
                                  style={{
                                    position: "absolute",
              ...
              {kcMode && !isGroup && (
                <>
                  ...
                  >
                    KC Mode{likelihoodMode && " + Likelihood"}
                  </Typography>
                  <Box
                    sx={{
                      display: "flex",
                  ...
                  >
                    <Button
                      variant="contained"
                      className="setting-button settings-toggle"
                      onClick={handleImportPetData}
                    >
                      Import Data
                    </Button>
                    <Button
                    ...
                    <Button
                      variant="contained"
                      className="setting-button settings-toggle"
                      onClick={() => setKcExportOpen((v) => !v)}
                    >
                      {kcExportOpen ? "Hide Export" : "Show Export"}
                    </Button>
                    <Button
                      variant="contained"
                      className="setting-button settings-toggle"
                      onClick={() => setKcTableOpen((v) => !v)}
                    >
                      {kcTableOpen ? "Hide Table" : "Show Table"}
                    </Button>
                  </Box>
                  {exportedKcData && (
                      ...
                      >
                        ...
                        >
                          {petOrder
                            .filter(
                              (petName) =>
                                getPetIconClass(petName, petCount.pets) ===
                                "obtained-pet-icon",
                            )
                            .map((petName) => {
                              const formatPetName = (name: string) => {
                              ...
                              const petImage = detailedMode
                                ? DetailedPet[
                                    formatPetName(
                                      petName,
                                    ) as keyof typeof DetailedPet
                                  ]
                                : InvyPet[
                                    formatPetName(
                                      petName,
                                    ) as keyof typeof InvyPet
                                  ];
                              ...
                              if (likelihoodMode) {
                                ...
                                if (petName === "Little nightmare") {
                                  ...
                                  const rateObj = petRates.find(
                                    (r) => r.pet === petName,
                                  );
                                  const dropRate = rateObj
                                    ? Number(rateObj.dropRate)
                                    : null;
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <TextField
                                    ...
                                    onChange={(e) =>
                                      handleKcChange(petName, e.target.value)
                                    }
                                    sx={{
                                      width: "90%",
                                      input: {
                                      ...
                                      {petName === "Little nightmare" ? (
                                        ...
                                        <>
                                          ...
                                          <TextField
                                            ...
                                            onChange={(e) =>
                                              handleYoungllefCorruptedKcChange(
                                                petName,
                                                e.target.value,
                                              )
                                            }
                                        ...
                                        <>
                                          ...
                                          <TextField
                                            ...
                                            onChange={(e) =>
                                              handleVenenatisSpindelKcChange(
                                                petName,
                                                e.target.value,
                                              )
                                            }
                                        ...
                                        <>
                                          ...
                                          <TextField
                                            ...
                                            onChange={(e) =>
                                              handleJalNibRekOfftaskKcChange(
                                                petName,
                                                e.target.value,
                                              )
                                            }
