...
export default function Pets() {
  ...
  const emptyPets: PetCountResponse = {
    pets: {},
    pet_hours: 0,
    pet_count: 0,
    player: " ",
  ...
  const proxyBases = (() => {
    const envProxy = (process.env.REACT_APP_CORS_PROXY_URL || "").trim();
    ...
    bases.push("https://corsproxy.io/?");
    bases.push("https://api.allorigins.win/raw?url=");

    return bases;
  })();

  const requestViaProxy = async (targetUrl: string) => {
    let lastError: unknown;

    for (const proxyBase of proxyBases) {
      try {
        return await axios.get(buildProxyUrl(proxyBase, targetUrl));
      } catch (error) {
        lastError = error;
  ...
  const getPetCount = async () => {
    ...
    try {
      if (!isGroup) {
        ...
        let response;
        response = await requestViaProxy(url);
        if (response && response.data.data) {
          ...
          chambers.forEach((item: any) => {
            if (item.id === 22386 && item.count >= 1) metaDust = 1;
          });
          // Sanguine Dust (25746) from theatre_of_blood
          tob.forEach((item: any) => {
            if (item.id === 25746 && item.count >= 1) sanguineDust = 1;
          });
          ...
          setPetCounts({
            "1": {
              ...
              player:
                data.player_name_with_capitalization || data.player || player,
              rank: 0,
            },
          });
        ...
        const response = await requestViaProxy(url + "?" + params.toString());
  ...
  const handleManualModeChange = (
    ...
    setPlayer(newPlayer);
    if (manualMode) {
      const newPetCount: PetCountResponse = {
        pets: {},
        pet_hours: 0,
        pet_count: 0,
        player: newPlayer,
  ...
  const handleBgColorChange1 = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    document.documentElement.style.setProperty(
      "--obtained-pet-bg",
      `linear-gradient(135deg, ${newColor}, ${petBgColor2})`,
    );
  };
  ...
  const handleBgColorChange2 = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    document.documentElement.style.setProperty(
      "--obtained-pet-bg",
      `linear-gradient(135deg, ${petBgColor1}, ${newColor})`,
    );
  };

  const handlePetCountColorChange = (
    event: React.ChangeEvent<HTMLInputElement>,
  ) => {
    setPetCountColor(event.target.value);
  };

  const handlePetHoursColorChange = (
    event: React.ChangeEvent<HTMLInputElement>,
  ) => {
  ...
  useEffect(() => {
    document.documentElement.style.setProperty(
      "--obtained-pet-bg",
      `linear-gradient(135deg, ${petBgColor1}, ${petBgColor2})`,
    );
    ...
    <Page title="3400 Pet List Tools">
      ...
      <Box sx={{ display: "flex", minHeight: "100vh" }}>
        ...
        {isMobile ? (
          <>
            ...
            >
              ...
              >
                ...
                <img src={nav3400rs} alt="icon" style={{ height: 32 }} />
                <Box />
              </Toolbar>
            </AppBar>
            <Drawer
              ...
              PaperProps={{
                sx: { backgroundColor: "#1b1a1d", width: 260, zIndex: 1300 },
              }}
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <ToggleButtonGroup
                    value={
                      isGroup ? "group" : asciiGen ? "ascii" : "individual"
                    }
                    exclusive
                  ...
                  >
                    <ToggleButton
                      ...
                      sx={{
                        flex: 1,
                        color: "white",
                        "&.Mui-selected": { color: petCountColor },
                      }}
                    >
                      <PersonOutlined />
                    </ToggleButton>
                  ...
                  {!asciiGen && (
                    <>
                      {!isGroup && (
                        <>
                          ...
                          <Button
                            variant="contained"
                            onClick={() => {
                              setLikelihoodMode(!likelihoodMode);
                            }}
                            className="setting-button settings-toggle"
                            sx={{
                              display: "flex",
                        ...
                        <>
                          <Button
                            ...
                            sx={{
                              ...
                              justifyContent: "space-between",
                              color: "#656566",
                              "&:hover": { backgroundColor: "#1b1d1a" },
                              "&:active": { backgroundColor: "#1b1d1a" },
                            }}
                            onClick={() => {
                              setMissingSettingsOpen(!missingSettingsOpen);
                            }}
                            disableRipple
                          >
                            <Typography>Missing Settings</Typography>
                            <ExpandMore
                              sx={{
                                transform: missingSettingsOpen
                                  ? "rotate(180deg)"
                                  : "rotate(0deg)",
                          ...
                          {missingSettingsOpen && (
                            <Zoom
                              in={missingSettingsOpen}
                              style={{
                                transitionDelay: missingSettingsOpen
                                  ? "100ms"
                                  : "0ms",
                              }}
                            >
                              ...
                              >
                                <Button
                                  variant="contained"
                                  onClick={() => {
                                    setMissingMode(!missingMode);
                                  }}
                                  className="setting-button"
                                  disabled={isLeaderboard ? true : false}
                                  sx={{
                                    mb: 3,
                                    display: "flex",
                                    alignItems: "center",
                                ...
                                <Button
                                  variant="contained"
                                  onClick={() => {
                                    setCombinedMissing(!combinedMissing);
                                  }}
                                  className="setting-button"
                                  disabled={isLeaderboard ? true : false}
                                  sx={{
                      ...
                      <Button
                        ...
                        onClick={() => {
                          setImageSettingsOpen(!imageSettingsOpen);
                        }}
                        disableRipple
                      >
                      ...
                      {imageSettingsOpen && (
                        <Zoom
                          in={imageSettingsOpen}
                          style={{
                            transitionDelay: imageSettingsOpen
                              ? "100ms"
                              : "0ms",
                          }}
                        >
                      ...
                      {colorSettingsOpen && (
                        <Zoom
                          in={colorSettingsOpen}
                          style={{
                            transitionDelay: colorSettingsOpen
                              ? "100ms"
                              : "0ms",
                          }}
                        >
                            ...
                            >
                              ...
                              <Box>
                                <input
                                  type="color"
                                  value={petCountColor}
                                  onChange={handlePetCountColorChange}
                                  style={{
                                    marginLeft: "10px",
                                    borderRadius: "90px",
                                    width: "30px",
                            ...
                            >
                              ...
                              <Box>
                                <input
                                  type="color"
                                  value={petHoursColor}
                                  onChange={handlePetHoursColorChange}
                                  style={{
                                    marginLeft: "10px",
                                    borderRadius: "90px",
                                    width: "30px",
                  ...
                  {asciiGen && (
                    <>
                      <Button
                        variant="contained"
                        onClick={() => {
                          setDiscordFormatting(!discordFormatting);
                        }}
                        className="setting-button settings-toggle"
                        sx={{
                          mb: 3,
          ...
          <Box
            sx={{
              ...
              pb: 3,
              zIndex: 1,
              borderBottom: "1px solid #2b2f30",
              borderRight: "1px solid #2b2f30",
            }}
          >
            <Box
              sx={{
                ...
                <>
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              ...
                              <Checkbox
                                checked={missingMode}
                                onChange={() => setMissingMode(!missingMode)}
                                color="default"
                                className="settings-toggle"
                              />
                            </Button>
                  ...
                  {imageSettingsOpen && (
                    <Zoom
                      in={imageSettingsOpen}
                      style={{
                        transitionDelay: imageSettingsOpen ? "100ms" : "0ms",
                      }}
                    >
                      <Box
                        sx={{
                  ...
                  {colorSettingsOpen && (
                    <Zoom
                      in={colorSettingsOpen}
                      style={{
                        transitionDelay: colorSettingsOpen ? "100ms" : "0ms",
                      }}
                    >
                      <Box
                        sx={{
        ...
        <Box
          sx={{
            flexGrow: 1,
            padding: isMobile ? "0 0.5rem" : "0 !important",
        ...
        >
            ...
            >
              ...
              <Box>
                <GradientText
                  ...
                  showBorder={false}
                >
                  <Typography sx={{ ml: 2, fontWeight: "500" }} variant="h4">
                    {asciiGen
                      ? "Ascii"
                      : isGroup
                        ? isLeaderboard
                ...
                {!isGroup && !asciiGen && (
                  <Typography sx={{ ml: 2, fontWeight: "300" }} variant="body2">
                    Enter your RuneScape username and click the magnifying
                    glass.
                    <br />
                    By querying the Temple&apos;s API the table will display
                    your pets below.
                    <br />
                    Use the sidebar options to customize the chart&apos;s visual
                    appearance.
                  </Typography>
                )}
                {isGroup && !asciiGen && (
                  <Typography sx={{ ml: 2, fontWeight: "300" }} variant="body2">
                    Enter your Group / Clans Temple Id and click the magnifying
                    glass.
                    <br />
                    This will query the Temple&apos;s API to retrieve and
                    display your groups pets in the chart below.
                    <br />
                    Use the sidebar options to customize the chart&apos;s visual
                    appearance or toggle between leaderboard and comparative
                    mode.
                  </Typography>
                )}
                {asciiGen && (
                  <Typography sx={{ ml: 2, fontWeight: "300" }} variant="body2">
                    Enter in your pet names, kill counts, and dates to generate
                    an ASCII table.
                    <br />
                    Use the sidebar options to customize the formatting for
                    displaying in different locations.
                    <br />
                    After clicking Generate Ascii Table the table will be copied
                    to your clipboard.
                    <br />
            ...
            >
              ...
              >
                ...
                >
                  <h3 style={{ margin: 0, fontSize: "1.2em" }}>
                    Error, No data found for this RSN.
                  </h3>
                  To have your pet data uploaded to Temple, it requires the
                  RuneLite plugin <b>TempleOSRS</b> to be installed and the{" "}
                  <b>Automatically sync Collection Log</b> option to be
                  selected.
                  <br />
                  For more information please refer to{" "}
                  <a
                    href="https://www.templeosrs.com/faq.php#FAQ_22"
                    target="_blank"
                    rel="noopener noreferrer"
                    style={{ color: "#ffb300" }}
                  >
                    this FAQ (Collection Log Section)
                  </a>
                  .
                </Alert>
              </Snackbar>
              <Typography>
                {isGroup ? "Temple Group Id" : "Runescape Name"}
              </Typography>
              <Paper
                component="form"
                sx={{
                  p: isMobile ? "2px 2vw" : "2px 4px",
                  display: "flex",
                  alignItems: "center",
              ...
              >
                ...
                <InputBase
                  ...
                  placeholder={
                    isGroup ? "Temple Group Id" : "RuneScape Username"
                  }
          ...
          {asciiGen && (
            <AsciiGenerator
              discordFormat={discordFormatting}
...
function setRows(
  rows: { id: number; pet: string; kc: string; date: string }[],
) {
